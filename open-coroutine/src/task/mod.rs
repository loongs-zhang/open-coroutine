...
extern "C" {
    fn task_crate(f: extern "C" fn(usize) -> usize, param: usize, stack_size: usize) -> JoinHandle;
}

#[allow(box_pointers)]
pub fn tk<F, P: 'static, R: 'static>(f: F, param: P, stack_size: usize) -> JoinHandle
where
    F: FnOnce(P) -> R + Copy,
{
...
macro_rules! tk {
    ...
    ( $f: expr , $param:expr ,$stack_size: expr $(,)?) => {{
        $crate::task::tk($f, $param, $stack_size)
    }};
}
