...
pub trait Monitor: Current<'static> {
...
thread_local! {
    static MONITOR: Cell<*const c_void> = Cell::new(std::ptr::null());
}

impl Current<'static> for MonitorImpl {
    #[allow(clippy::ptr_as_ptr)]
    fn init_current(current: &Self)
    where
        Self: Sized,
    {
        MONITOR.with(|c| c.set(current as *const _ as *const c_void));
    }
        ...
        Self: Sized,
    {
        MONITOR.with(|boxed| {
            ...
            if ptr.is_null() {
                ...
                Some(unsafe { &*(ptr).cast::<MonitorImpl>() })
            }
        })
    }
    ...
    where
        Self: Sized,
    {
        MONITOR.with(|boxed| boxed.set(std::ptr::null()));
    }
}
            ...
            *monitor = MaybeUninit::new(
                std::thread::Builder::new()
                    ...
                    .spawn(|| {
                        ...
                        MonitorImpl::init_current(monitor);
                        // thread per core
