...
pub trait StateCoroutine<'c>: Coroutine<'c> {
    ...
    fn change_state(&self, state: CoroutineState<Self::Yield, Self::Return>);
    ...
    fn ready(&self) -> std::io::Result<()> {
        ...
        match current {
            CoroutineState::Created => {
                self.change_state(CoroutineState::Ready);
                return Ok(());
            }
    ...
    fn running(&self) -> std::io::Result<()> {
        ...
        match current {
            #[cfg(test)]
            CoroutineState::Created => {
                self.change_state(CoroutineState::Running);
                return Ok(());
            }
    ...
    fn suspend(&self, val: Self::Yield, timestamp: u64) -> std::io::Result<()> {
        ...
        if CoroutineState::Running == current {
            self.change_state(CoroutineState::Suspend(val, timestamp));
            return Ok(());
        }
        Err(Error::new(
            ErrorKind::Other,
    ...
    fn complete(&self, val: Self::Return) -> std::io::Result<()> {
        let current = self.state();
        if CoroutineState::Running == current {
            self.change_state(CoroutineState::Complete(val));
            return Ok(());
        }
        Err(Error::new(
            ErrorKind::Other,
    ...
    fn error(&self, val: &'static str) -> std::io::Result<()> {
        let current = self.state();
        if CoroutineState::Running == current {
            self.change_state(CoroutineState::Error(val));
            return Ok(());
        }
        Err(Error::new(
            ErrorKind::Other,
