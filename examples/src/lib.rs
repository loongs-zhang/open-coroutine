...
pub fn crate_server(
    ...
    for stream in listener.incoming() {
        ...
        loop {
            ...
            assert_eq!(512, bytes_read, "read less than expect");
...
pub fn crate_client(port: u16, server_started: Arc<AtomicBool>) {
    ...
    _ = stream
        .peer_addr()
        .unwrap_or_else(|_| panic!("Couldn't connect to 127.0.0.1:{port} !"));
    let mut data: [u8; 512] = unsafe { std::mem::zeroed() };
    data[511] = b'\n';
    ...
    assert_eq!(
        1,
        stream.write(&[b'e']).expect("Failed to write!"),
        "write not expect"
    );
...
pub fn sleep_test(millis: u64) {
    ...
    let end = now();
    assert!(
        end - start >= millis * 1_000_000,
        "Time consumption less than expected"
    );
}
